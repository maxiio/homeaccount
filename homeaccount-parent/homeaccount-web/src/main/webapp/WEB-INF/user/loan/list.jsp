			...
			function delByIds(){
				...
				//批量删除
				 Ewin.confirm({ message:"你确定删除这"+nums+"条数据？" }).on(function (e) {
	                 if (e) {
	                  location.href="${pageContext.request.contextPath}/LoanAction_deletebatch.action?loan_ids="+s.substring(0,s.length-1);
	                }
					});
	...
	<div class="container-fluid">
		...
		<div class="well">
			<form class="form-inline" method="post"
				action="${pageContext.request.contextPath }/LoanAction_list.action">
				<b>根据借款条件搜索</b> 
				借款名称:<input
					style="margin-right: 25px;" type="text" class="form-control"
					 value="<s:property value="loanname"/>"
					name="loanname" />
			...
			<BR />
				...
				<a class="btn btn-success"
					href="${pageContext.request.contextPath }/page_user_loan_add.action"><span
		...
		<s:else>
			<table class="table table-bordered table-striped">
				...
				<tbody>
					<s:iterator value="#pageBean.list" var="loan" status="index">
						<tr>
							<td align="center"><input type="checkbox" name="valueBox"
								id="valueBox"
								value="<s:property value="#loan.loan_id"/>" /></td>
							<td><s:property value="#index.count" /></td>
							<td><s:property
									value="#loan.loanname" /></td>
							<td><s:property value="#loan.loan_source" /></td>
							<td><s:property value="#loan.loan_datetime_s" /></td>
							<td><s:property value="#loan.loan_year" /></td>
							<td><s:property value="#loan.loan_money" /></td>
							<td><s:property value="#loan.interest_rates" />%</td>
							<td><s:property value="#loan.lixi" /></td>
							<td><s:property value="#loan.loan_createtime_s" /></td>
							<td><s:property value="#loan.overtime" escape="false"/></td>
							<td style="font-size: 16px;"><a class="label label-danger"
								href="javascript:delInAccountById('<s:property value="#loan.loan_id"/>')">删除</a>
								<a class="label label-primary" href="javascript:;"
								onclick="openData('<s:property value="#loan.loan_id"/>')">修改</a>
							</td>
						</tr>
					</s:iterator>
				</tbody>
			...
			<div align="center">
				<nav aria-label="Page navigation">
					<ul class="pagination">
						...
						<c:if test="${pageBean.currentPage>1}">
							<li class=""><a
								href="${pageContext.request.contextPath }/LoanAction_list?currentPage=1">首页</a></li>
							<li class=""><a
								href="${pageContext.request.contextPath }/LoanAction_list?currentPage=${pageBean.currentPage-1}"
						...
						<c:forEach var="i" begin="${begin }" end="${end }">
							<c:choose>
								...
								<c:otherwise>
									<li><a
										href="${pageContext.request.contextPath }/LoanAction_list?currentPage=${i}"
										onclick="">${i }</a></li>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<c:if test="${pageBean.currentPage<pageBean.totalPage }">
							...
							<li><a
								href="${pageContext.request.contextPath }/LoanAction_list?currentPage=${pageBean.totalPage}">尾页</a></li>
						</c:if>
						<c:if test="${pageBean.currentPage==pageBean.totalPage }">
			...
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					...
					<div class="modal-body" align="center">
						<form id="updateForm" action="${pageContext.request.contextPath }/LoanAction_update.action" method="post">
							...
							<div class="form-group form-inline">
								...
								<input class="form-control" type="text" style="width: 240px;" name="loanname" required="required"/>
							</div>
							<div class="form-group form-inline">
	...
	<script>
		...
		function delInAccountById(id){
			  Ewin.confirm({ message: "确认要删除编号为"+id+"的数据吗？" }).on(function (e) {
                 if (e) {
                   location.href="${pageContext.request.contextPath}/LoanAction_delete.action?loan_id="+id;
                }
				});
		}
		function closeData(){	
		...
		function openData(id){
				//手动开启
				 ...
				 //填充模态框表单
				 $.post("${pageContext.request.contextPath}/LoanAction_findById.action",{"loan_id":id},function(data){
