...
<script>
		 //单个删除
			function deloutaccounttypeById(id){
				  Ewin.confirm({ message: "确认要删除编号为"+id+"的数据吗？" }).on(function (e) {
	                 if (e) {
	                	 $.post("${pageContext.request.contextPath}/OutAccountTypeAction_delete.action",{"outaccounttype_id":id},function(data){
	                		 if(data=="1"){
	                			 Ewin.alert({message:"删除成功！"})
	                			 window.location.reload();
	                		 }else{
	                			 Ewin.alert({message:"删除出错（可能是该类型下有账单）！"})
	                		 }
	                	 });
	                }
					});
	...
	<body>
	   ...
	   <div class="container-fluid">
		...
		<!--路径墙end-->
			<!--搜索墙-->
		<div class="well">
			<form class="form-inline" action="${pageContext.request.contextPath }/OutAccountTypeAction_list.action" method="post">
				<span class=""><b>根据支出账单类型条件搜索</b></span>
				<strong style="margin-left: 20px;">支出类型名称:</strong>
				<input placeholder="输入收入账单类型名称" style="margin-right: 25px;" value="<s:property value="outaccounttype_name"/>" type="text" class="form-control" id="outaccounttype_name" name="outaccounttype_name"/>
				<button class="btn btn-primary" id="search" type="submit"><span class="glyphicon glyphicon-search"></span>搜索</button>
				<a class="btn btn-success pull-right" href="${pageContext.request.contextPath }/page_user_outaccounttype_add.action"><span class="glyphicon glyphicon-plus"></span>添加支出账单类型</a>
			</form>
		</div>
		<!--搜索墙end-->
		...
		<table class="table table-bordered table-striped">
				...
				<tbody>
					<s:iterator var="outtype" value="#pageBean.list" status="index">
						
						<tr>
							<td><s:property value="#index.count"/></td>
							<td><s:property value="#outtype.outaccounttype_name"/></td>
							<td><s:property value="#outtype.outaccounts.size"/>条</td>
							<td><s:property value="#outtype.outaccounttype_desc"/></td>
							<td><s:property value="#outtype.outaccounttype_createtime_s"/></td>
							<td style="font-size: 16px;">
								<a class="label label-danger" href="javascript:deloutaccounttypeById(<s:property value="#outtype.outaccounttype_id"/>)">删除</a>
								<a class="label label-primary" href="javascript:openData(<s:property value="#outtype.outaccounttype_id"/>)">修改</a>
							</td>
						</tr>
					</s:iterator>
				</tbody>
			...
			<div align="center">
				<nav aria-label="Page navigation">
					<ul class="pagination">
						...
						<c:if test="${pageBean.currentPage>1}">
							<li class=""><a
								href="${pageContext.request.contextPath }/OutAccountTypeAction_list?currentPage=1">首页</a></li>
							<li class=""><a
								href="${pageContext.request.contextPath }/OutAccountTypeAction_list?currentPage=${pageBean.currentPage-1}"
						...
						<c:forEach var="i" begin="${begin }" end="${end }">
							<c:choose>
								...
								<c:otherwise>
									<li><a
										href="${pageContext.request.contextPath }/OutAccountTypeAction_list?currentPage=${i}"
										onclick="">${i }</a></li>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<c:if test="${pageBean.currentPage<pageBean.totalPage }">
							<li class=""><a
								href="${pageContext.request.contextPath }/OutAccountAction_list?currentPage=${pageBean.currentPage+1}"
								aria-label="Next"> 下一页</a></li>
							<li><a
								href="${pageContext.request.contextPath }/OutAccountTypeAction_list?currentPage=${pageBean.totalPage}">尾页</a></li>
						</c:if>
						<c:if test="${pageBean.currentPage==pageBean.totalPage }">
			...
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					...
					<div class="modal-body" align="center">
						<form id="updateForm" action="${pageContext.request.contextPath }/OutAccountTypeAction_update.action" method="post">
		...
		function openData(id){
				//手动开启
				 ...
				 //填充模态框表单
				 $.post("${pageContext.request.contextPath}/OutAccountTypeAction_findById.action",{"outaccounttype_id":id},function(data){
